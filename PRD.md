...
| `user.md` | 主题·人 | 用户偏好、身份、工作风格、目标 | **用户 + agent** | 常驻注入 |
| `agent.md` | 主题·我 | 智能体自我认知、踩坑教训、有效工作方式 | agent（用户可改） | 常驻注入 |
| `project.md` | 主题·项目 | 项目上下文、约定、决策、当前进度 | agent + 用户 | 常驻注入（当前项目） |
| `memory.md` | 主题·世界 | 通用事实/决策/知识条目（兜底） | agent | 按需 BM25 检索 |
...
### 3.1 归属判定链（先问"关于谁/什么"，按序落盘）

```
一条新信息
 ├─ 1. 它是"某天发生的事"（时间锚定）？      → daily/<date>.md
 ├─ 2. 它关于"用户这个人"（跨场景成立）？    → user.md
 ├─ 3. 它关于"某个项目"（仅该项目成立）？    → projects/<proj>/project.md
 ├─ 4. 它是"agent 如何工作"（过程性）？      → agent.md
 └─ 5. 都不是                                → memory.md（世界/通用知识，兜底）
记忆系统自身的变更（整合报告）               → dream.md（不参与此链）
```
...
| 文件 | 放（唯一原则） | 不放（去哪） |
|---|---|---|
| `daily/<date>.md` | **时间锚定**的内容：今日要点、会话纪要、今日待办、当日产出 | 持久事实（→ 主题文件）、整合报告（→ dream.md）、细节（→ 会话日志兜底） |
| `user.md` | **关于用户这个人且跨场景成立**：身份背景、偏好（工具/语言/节奏）、目标、禁忌、对 agent 的期望、个人想法 | 仅某项目生效的偏好（→ project.md）、通用知识（→ memory.md）、agent 的事（→ agent.md） |
| `agent.md` | **过程性（how to work）**：身份与能力边界、工作方式（有效/无效）、经验教训、环境工作层怪癖 | 事实性知识 what is（→ memory.md）、项目细节（→ project.md）、用户偏好（→ user.md） |
| `project.md` | **只关于该项目**：目标、技术栈/架构、约定、决策及理由、当前进度、坑与规避、项目内偏好 | 跨项目通用知识（→ memory.md）、个人偏好（→ user.md）、时间事件（→ daily） |
| `memory.md` | **兜底**：领域知识、通用决策、学习到的做法、通用想法 | 有归属的信息（→ 按判定链前三项） |
| `dream.md` | **审计日志**：每次整合的变更清单 + 原因 | 任何记忆内容本身 |

**两条硬边界**：
- *事实性 vs 过程性*：世界是怎样的（what is）→ memory.md；怎么做更有效（how to work）→ agent.md。
- *全局 vs 项目*：跨场景成立 → user.md/memory.md；仅某项目成立 → project.md。

### 3.3 文件模板（边界可执行化）

```markdown
# user.md
## 身份与背景        ## 偏好（工具/语言/风格）   ## 目标
## 禁忌与边界        ## 想法

# agent.md
## 身份与能力边界    ## 工作方式（有效/无效）    ## 经验教训

# projects/<proj>/project.md
## 目标              ## 技术栈与架构            ## 约定
## 决策记录（含理由） ## 当前进度                ## 坑与规避
...
# memory.md          条目式：## 小节，主题作标题，首行为条目主题
# dream.md           按次追加：## <日期> dream #N —— 变更清单（合并/删除/新增/归档 + 原因）
```
...
| "你是谁 / 偏好什么" | user.md |
| "不要做 X"（禁忌） | user.md |
| "今天 / 昨天 / 上周发生了什么" | daily/<date>.md（日期检索） |
| "进行到哪了"（跨天续接） | daily 今日要点 + project.md 进度 |
| "项目目标 / 约定 / 决策为什么" | project.md |
| "这个项目怎么最顺 / 坑" | project.md 坑 + agent.md 教训 |
| "这个环境怎么工作" | agent.md（过程层） |
| "某个领域的知识" | memory.md |
| "我有个想法" | 按归属：user / project / memory 的 ## 想法 |
| "试过什么没用" | agent.md 教训 / project.md 坑 |
| "记忆为什么变了" | dream.md |
| 临时上下文（不需持久） | 不存——会话日志（ground truth）兜底 |
